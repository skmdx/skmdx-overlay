...
src_unpack() {
	...
	local module cache_dir version
	for module in "${MY_GO_MODULE_FILES[@]}"; do
		cache_dir="${T}/go-proxy/${module%@*}/@v"
		mkdir -p "${cache_dir}" || die
		cp "${DISTDIR}/${module//\//_}" "${cache_dir}/${module#*@}" || die
		if [[ ${module} == *.mod ]]; then
			# Go reserializes cached .info JSON, so it is not a stable distfile.
			# Pinned versions only need Version; Time is optional in GOPROXY.
			version=${module#*@}
			version=${version%.mod}
			printf '{"Version":"%s"}\n' "${version}" > "${cache_dir}/${version}.info" || die
		fi
	done
...
src_test() {
	# The ANSI style tests require colors regardless of the invoking shell.
	unset NO_COLOR
